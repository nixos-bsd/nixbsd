...
let
  ...
  lateBuildPartition =
    i: part:
    if part.tooLargeIntermediate or false then
      ''
  ...
  fixPartition =
    ...
    // {
      filepath =
        if part.tooLargeIntermediate or false then "$TMP/latebuild/${builtins.toString i}" else part;
    };
  partitionsFixed = lib.imap1 fixPartition partitions;

  lateBuildNativeInputs = lib.flatten (
    builtins.map (
      part: if part.tooLargeIntermediate or false then part.nativeBuildInputs else [ ]
    ) partitions
  );
  ...
  partitionDiskScript =
    {
      ...
      efi =
        let
          partitionFlags = lib.concatMapStringsSep " " (
            part:
            let
              aliasMap = {
                "efi" = "efi";
              ...
              filename =
                if part ? filepath then
                  part.filepath
                else if part ? filename then
                  "${part}/${part.filename}"
                else
                  "${part}";
            in
            "-p ${aliasMap.${part.filesystem or part.partitionTableType}}/${part.label}:=${filename}"
          ) partitionsFixed;
      ...
      bsd =
        let
          partitionFlags = lib.concatMapStringsSep " " (
            ...
            in
            "-p ${aliasMap.${part.filesystem or part.partitionTableType}}:=${filename}"
          ) partitions;
          sizeFlags = lib.optionalString (totalSize != null) "--capacity ${totalSize}";
