...
# !!! system can be set modularly, would be nice to remove,
{
  ...
  system ? builtins.currentSystem, # !!! is this argument needed any more? The pkgs argument can
  # be set modularly anyway.
  pkgs ? null, # !!! See comment about args in lib/modules.nix
  extraArgs ? { }, # !!! See comment about args in lib/modules.nix
  specialArgs ? { },
  modules,
  modulesLocation ? (builtins.unsafeGetAttrPos "modules" evalConfigArgs).file or null, # !!! See comment about check in lib/modules.nix
  check ? true,
  prefix ? [ ],
  lib ? import <nixpkgs/lib>, # Not imported, we want the actual path to be passed in
  nixpkgsPath ? <nixpkgs>,
  extraModules ?
    let
      e = builtins.getEnv "NIXBSD_EXTRA_MODULE_PATH";
    in
...
let
  ...
  evalModulesMinimal = (import ./eval-config-minimal.nix { inherit lib; }).evalModules;
  ...
  nixbsdWithUserModules = noUserModules.extendModules { modules = allUserModules; };
...
in
withWarnings (withExtraAttrs nixbsdWithGender)
