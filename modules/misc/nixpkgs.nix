...
let
  ...
  stdenvArgs = {
    name = "stdenv-lies";
    shell = lib.getExe finalPkgs.bash;
    ...
    hostPlatform = finalPkgs.stdenv.hostPlatform;
...

{
  ...
  options.nixpkgs = {
    fakeNative = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Commit crimes to allow trivial builders to run on platforms with no stdenv";
    };
  ...
  config = {
    nixpkgs._hostPlatform = finalPkgs.stdenv.hostPlatform;
    _module.args = {
      pkgs =
