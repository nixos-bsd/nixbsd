...
let
  ...
  mkFstab =
    name: cfg:
    pkgs.writeText "jail.${name}.fstab" (
      ''
        # Device''\tMountpoint''\tFStype''\tOptions''\tDump''\tPass#
      ''
      + lib.concatMapStrings (mnt: ''
        ${
          if mnt.hostPath == null then mnt.mountPoint else mnt.hostPath
        }''\t${stateDirectory}/${name}${mnt.mountPoint}''\tnullfs''\t${
          if mnt.isReadOnly then "ro" else "rw"
        }''\t0''\t2
...
in
{
                              ...
                              config = {
                                nixpkgs =
                                  if options.nixpkgs ? hostPlatform then
                                    { inherit (host.pkgs.stdenv) hostPlatform buildPlatform; }
                                  else
                                    {
                                      localSystem = host.pkgs.stdenv.buildPlatform;
                                      crossSystem = host.pkgs.stdenv.hostPlatform;
                                    };
                                boot.isJail = true;
  ...
  config = mkMerge [
    {
      warnings =
        optional (!config.boot.enableJails && config.jails != { })
    ...
    (mkIf config.boot.enableJails {
      freebsd.rc.services = lib.mapAttrs (
        name: cfg:
        let
          conf = mkJailConf name cfg;
        in
        {
          name = "jail_${name}";
      ...
      systemd.tmpfiles.settings = lib.mapAttrs' (
        name: conf:
        lib.nameValuePair "jail_${name}" (
          {
            "${stateDirectory}/${name}".d = { };
            "${stateDirectory}/${name}/dev".d = { };
            "${stateDirectory}/${name}/tmp".d = {
              mode = "0777";
            };
          }
          // (lib.mergeAttrsList (
            ...
            }) (lib.attrValues conf.bindMounts)
          ))
          // (lib.mergeAttrsList (
            map (mnt: {
              "${stateDirectory}/${name}${mnt}".d = {
                user = "root";
              };
            }) conf.tmpfs
          ))
