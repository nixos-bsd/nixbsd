...
my $cpuinfo = read_file "/etc/compat/linux/proc/cpuinfo";
...
foreach my $path (glob "/etc/compat/linux/sys/bus/pci/devices/*") {
    pciCheck $path;
}
...
foreach my $path (glob "/etc/compat/linux/sys/bus/usb/devices/*") {
...
foreach my $path (glob "/etc/compat/linux/sys/class/{block,mmc_host}/*") {
    my $module;
...
# devices.
my @swaps = read_file("/etc/compat/linux/proc/swaps", err_mode => 'carp');
my @swapDevices;
if (@swaps) {
    shift @swaps;
...
my %fsByDev;
my $useSwraid = 0;
foreach my $fs (read_file("/etc/compat/linux/proc/self/mountinfo")) {
    chomp $fs;
    my @fields = split / /, $fs;
...
EOF
    ...
    if ($device =~ /\/dev\/loop(\d+)/) {
        my $loopnr = $1;
        my $backer = read_file "/etc/compat/linux/sys/block/loop$loopnr/loop/backing_file";
        if (defined $backer) {
            chomp $backer;
            $device = $backer;
...
EOF
    ...
    if (-e $device) {
        ...
        my $dmUuid = read_file("/etc/compat/linux/sys/class/block/$deviceName/dm/uuid",  err_mode => 'quiet');
        if ($dmUuid =~ /^CRYPT-LUKS/)
        {
            my @slaves = glob("/etc/compat/linux/sys/class/block/$deviceName/slaves/*");
            if (scalar @slaves == 1) {
                my $slave = "/dev/" . basename($slaves[0]);
                if (-e $slave) {
                    my $dmName = read_file("/etc/compat/linux/sys/class/block/$deviceName/dm/name");
                    chomp $dmName;
        ...
        if (-e "/etc/compat/linux/sys/class/block/$deviceName/md/uuid") {
            $useSwraid = 1;
        }
    }
}
...
EOF
    ...
    if ($force || ! -e $fn) {
        ...
        my $bootLoaderConfig = "";
        if (-e "/etc/compat/linux/sys/firmware/efi/efivars") {
