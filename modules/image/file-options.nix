...
}:
{
  options.image = {
    baseName = lib.mkOption {
      type = lib.types.str;
      default = "nixbsd-image-${config.system.nixos.label}-${pkgs.stdenv.hostPlatform.system}";
      defaultText = lib.literalExpression "nixbsd-image-\${config.system.nixos.label}-\${pkgs.stdenv.hostPlatform.system}";
      description = ''
