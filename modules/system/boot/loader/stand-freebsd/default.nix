...
let
  ...
  nixStoreSuffix =
    if config.boot.copyKernelToBoot then
      "/not/used"
    else
      lib.strings.removePrefix nixStoreMount nixStorePath;
  copyKernelsArg = lib.optionalString config.boot.copyKernelToBoot "-C";
  symlinkBootArg = lib.optionalString (config.virtualisation.netMountBoot or false) "-L";
