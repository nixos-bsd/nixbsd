...
let
  ...
  formatFlags =
    ...
    let
      origFlags =
        optionalString binary "B"
        + optionalString create "C"
        + optionalString noDump "D"
        + optionalString noEmptyRotate "E"
        + optionalString isGlob "G"
        + optionalString noSignal "N"
        + optionalString noCompressFirst "P"
        + optionalString alternateMessage "T"
    ...
    in
    if origFlags == "" then "-" else origFlags;
  ...
  formatOwner =
    ...
    in
    if owner == "" && group == "" then "" else "${owner}:${group}";
    ...
    let
      ...
      pidCmdFile =
        if cfg.pidFile != null then
          ...
          "";
    in
    "${cfg.path} ${owner} ${cfg.mode} ${toString cfg.count} ${size} ${when} ${flags} ${pidCmdFile} ${signal}";
...
in
{
  ...
  config = {
    ...
    services.newsyslog.logfiles = mkIf cfg.createDefault (
      mapAttrs (
        name: value:
        mkDefault {
