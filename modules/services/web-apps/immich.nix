...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    init.services.postgresql.postStart =
      let
    ...
    init.services.immich-machine-learning = mkIf cfg.machine-learning.enable {
      ...
      startType = "foreground";
      startCommand = [ (lib.getExe (cfg.package.machine-learning.override { immich = cfg.package; })) ];
      user = cfg.user;
    ...
    users.users = mkIf (cfg.user == "immich") {
      immich = {
        name = "immich";
        group = cfg.group;
        extraGroups = mkIf (cfg.redis.enable && isRedisUnixSocket) [
