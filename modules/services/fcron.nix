      ...
      type = lib.types.attrsOf (
        lib.types.submodule (
          { name, config, ... }:
          {
            ...
            config = let
              options = lib.concatStringsSep "," (lib.concatAttrsToList (opt: val: "${opt}(${if builtins.isBool val then (if val then "true" else "false") else builtins.toString val})") config.options);
              command = if builtins.isList config.command then lib.escapeShellArgs config.command else config.command;
            in lib.mkMerge [ (lib.mkIf (config.elapsed == null) {
              line = "@${options} ${config.elapsed} ${command}";
            }) (lib.mkIf (config.clock != null) {
              line = "&${options} ${config.elapsed} ${command}";
            }) {
