    ...
    init.services.postgresql = {
      ...
      preStart = ''
        if ! test -e ${cfg.dataDir}/PG_VERSION; then
          ...
          rm -f ${cfg.dataDir}/{*.conf,.first_startup}

          # Initialise the database.
          sudo -E -u postgres PGDATA=${cfg.dataDir} initdb -U ${cfg.superUser} ${escapeShellArgs cfg.initdbArgs}

          # See postStart!
          sudo -u postgres touch "${cfg.dataDir}/.first_startup"
        fi

        sudo -u postgres ln -sfn "${configFile}/postgresql.conf" "${cfg.dataDir}/postgresql.conf"
        ${optionalString (cfg.recoveryConfig != null) ''
          sudo -u postgres ln -sfn "${pkgs.writeText "recovery.conf" cfg.recoveryConfig}" \
