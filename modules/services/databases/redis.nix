...
let
  ...
  redisName = name: "redis" + lib.optionalString (name != "") ("_"+name);
...
in {
  ...
  config = lib.mkIf (enabledServers != {}) {
    ...
    init.services = lib.mapAttrs' (name: conf: lib.nameValuePair (redisName name) {
      ...
      preStart = let script = pkgs.writeShellScript "${redisName name}-prep-conf" (let
    ...
    systemd.tmpfiles.settings = lib.mapAttrs' (name: conf: lib.nameValuePair (redisName name) {
      "/var/lib/${redisName name}".d = { user = conf.user; };
      "/run/${redisName name}".d = { user = conf.user; };
    }) enabledServers;
  };
}
