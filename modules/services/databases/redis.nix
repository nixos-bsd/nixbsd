  ...
  config = lib.mkIf (enabledServers != { }) {
    ...
    init.services = lib.mapAttrs' (
      ...
      lib.nameValuePair (redisName name) {
        ...
        startCommand = [
          "${cfg.package}/bin/${cfg.package.serverBin or "redis-server"}"
          "/var/lib/${redisName name}/redis.conf"
        ]
        ++ conf.extraParams;
        preStart =
          let
            script = pkgs.writeShellScript "${redisName name}-prep-conf" (
              let
    ...
    systemd.tmpfiles.settings = lib.mapAttrs' (
      name: conf:
      lib.nameValuePair (redisName name) {
        "/var/lib/${redisName name}".d = {
          user = conf.user;
        };
        "/run/${redisName name}".d = {
          user = conf.user;
        };
      }
    ) enabledServers;
